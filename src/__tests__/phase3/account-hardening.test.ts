...
const ids: string[] = []
afterEach(async () => { if (dbOk) await db.user.deleteMany({ where: { id: { in: ids.splice(0) } } }) })
...
describe('account hardening', () => {
  it.skipIf(!dbOk)('blocks bootstrap administrators from admin APIs until password change', async () => {
  ...
  it.skipIf(!dbOk)('revokes refresh tokens even when access authentication is absent', async () => {
  ...
  it.skipIf(!dbOk)('clears the bootstrap flag when the initial password is changed', async () => {
    const user = await admin(true)
    const token = await signAccessToken({ id: user.id, email: user.email, name: user.name, role: 'ADMIN', status: 'ACTIVE', mustChangePassword: true })
